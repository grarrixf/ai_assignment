            ...
            if num_recommended_books > 0:
                ...
                if recommended_books_indices:
                    ...
                    if recommended_books_indices:
                        ...
                        genre_recommended_books['percentage'] = genre_recommended_books.apply(lambda x: st.session_state.cart[next((i for i, item in enumerate(st.session_state.cart) if item['title'] == x['title']), {'quantity': 0})]['quantity'] / total_quantity if x['title'] in [item['title'] for item in st.session_state.cart] else 0, axis=1)
